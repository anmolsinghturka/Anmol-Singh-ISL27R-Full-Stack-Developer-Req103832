...
exports.updateProductById = (req, res) => {
  try {
    ...
    let product = products.find((product) => product.contactId === contactId);

    // If the product is found, update it with the new data and write the updated product list to the data file
    if (product) {
      // Remove the contactId field from the request body
      const { contactId: _, ...updatedFields } = req.body;
      ...
      const updatedProducts = products.map((p) =>
        p.contactId === contactId ? product : p,
      );
      ...
      res
        .status(404)
        .json({ message: `Product with ID ${contactId} not found` });
    }
  } catch (error) {
