...
exports.createProduct = (req, res) => {
  try {
    ...
    // Remove contactId from req.body if it's present
    const { contactId: _, ...productData } = req.body;

    // Read the existing products from the data file
    const products = JSON.parse(fs.readFileSync(dataFilePath, "utf-8"));

    // Find the highest contactId and increment it by 1 to create a new contactId
    const highestcontactId = products.reduce((max, product) => {
      return Math.max(max, product.contactId);
    }, 0);

    // Create the new product object with a new contactId
    const product = {
      contactId: highestcontactId + 1,
      ...productData,
    };
